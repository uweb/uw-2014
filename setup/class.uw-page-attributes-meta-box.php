...
class UW_Page_Attributes_Meta_Box
{
    ...
    public function page_template_dropdown($default, $post)
    {
        $previews = array(
        ...
        echo "<p><input type='radio' name='page_template' value='default' $checked >Default Template</input> (<a id='enchanced-preview' href='#'>preview<span><img src='" . get_template_directory_uri() . $previews['Default Template'] . "' alt='' width='300px' height='' /></span></a>)</p>";
        foreach (array_keys($templates) as $template) {
            ...
            echo "<p><input type='radio' name='page_template' value='" . $templates[$template] . "' $checked >$template</input> " . (array_key_exists($template, $previews) ? "(<a id='enchanced-preview' href='#'>preview<span><img src='" . get_template_directory_uri() . $previews[$template] . "' alt='' width='300px' height='' /></span></a>)" : "") . "</p>";
        }
        echo "</div>";
    ...
    public function save_postdata($post_ID = 0)
    {
        ...
        if (isset($_POST['banner_name'])) {
            if (!empty($_POST) && check_admin_referer('banner_nonce', 'banner_name')) {
                if ($post_type) {
        ...
        if (isset($_POST['buttontext_name'])) {
            if (!empty($_POST) && check_admin_referer('buttontext_nonce', 'buttontext_name')) {
                if ($post_type) {
        ...
        if (isset($_POST['buttonlink_name'])) {
            if (!empty($_POST) && check_admin_referer('buttonlink_nonce', 'buttonlink_name')) {
                if ($post_type) {
        ...
        if (isset($_POST['mobileimage_name'])) {
            if (!empty($_POST) && check_admin_referer('mobileimage_nonce', 'mobileimage_name')) {
                if ($post_type) {
        ...
        if (isset($_POST['sidebar_name'])) {
            if (!empty($_POST) && check_admin_referer('sidebar_nonce', 'sidebar_name')) {
                if ($post_type) {
        ...
        if (isset($_POST['parent_name'])) {
            if (!empty($_POST) && check_admin_referer('parent_nonce', 'parent_name')) {
                if ($post_type) {
